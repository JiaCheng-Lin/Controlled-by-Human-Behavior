...
def main():
    ...
    click_permission = True ## cool down the mouse.click

    EYE_AR_THRESH = 0.15
    EYE_AR_CONSEC_FRAMES = 2  # consecutive 3 frame
    eye_COUNTER = 0 # eye counter (frame)
    eye_TOTAL = 0 # total eye blink
    Double_blink = False
    pre_blink_time = 0
    ...
    console = Console() # print colorfully
    ...
    while cap.isOpened():
        ...
        # first two steps
        img_facemesh, faces = detector.findFaceMesh(img, draw=True)
        ...
        if faces:
            ...
            """control mouse with"""
            ## convert coordinates
            left_right = np.clip(yaw*-1, -35, 35)
            up_down = np.clip(40-pitch, 25, 43) 
            x = np.interp(left_right, (-35, 35), (0, 1920)) # *-1: filp
            y = np.interp(up_down, (25, 43), (0, 1080))
            # cv2.putText(img_facemesh, str(pitch), (20, 150), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 0, 255), 1)
            
            # smooth value
            smoothening = 6 # larger->smooth & steady but lag, smaller->sensitive but not steady.
            cur_loc_x = pre_loc_x + (x - pre_loc_x) / smoothening
            cur_loc_y = pre_loc_y + (y - pre_loc_y) / smoothening

            # move mouse
            mouse.move(cur_loc_x, cur_loc_y)
            # cv2.putText(img_facemesh, "width:%d, height:%d"%(pre_loc_x, pre_loc_y), (20, 150), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 0, 255), 1)
            pre_loc_x, pre_loc_y = cur_loc_x, cur_loc_y

            if click_permission == False:
                    click_clock_end = datetime.now()
                    if (click_clock_end-click_clock_start).total_seconds() >= 1:
                        click_permission = True

            #### mouse click
            # # MouthOpening
            # if mar > 0.3 and click_permission: # if mouse.click once, then cool down for 1 sec
            #     click_permission = False
            #     mouse.click()
            #     click_clock_start = datetime.now()
            ...
            # threshold
                ...
                if eye_COUNTER >= EYE_AR_CONSEC_FRAMES:  # blink!
                    # mouse.click()

                    # # count the diff time between blinks
                    if pre_blink_time == 0:
                        pre_blink_time = datetime.now()
                    else:
                        now_blink_time = datetime.now()
                        if (now_blink_time-pre_blink_time).total_seconds() <= 0.5:
                            Double_blink = True
                        else:
                            Double_blink = False
                        pre_blink_time = now_blink_time

                    ## pdf scroll control.
                    if Double_blink: # double blink to control
                        if freezeScroll: 
                            freezeScroll=False
                        else:
                            mouse.wheel(delta=0)
                        ...
                        console.print("Double_blink", style='red')
            ...
            """ pdf scroll speed controlled by pitch value """
            if not freezeScroll:  
                if pitch < -5: # down
                    mouse.wheel(delta=-0.2)
                elif pitch >=-5 and pitch<=3: # down
                    mouse.wheel(delta=-0.2) 
                elif pitch>=7 and pitch < 10: # up
                    mouse.wheel(delta=0.2)
                elif pitch >= 10: # up
                    mouse.wheel(delta=0.2)

                cv2.putText(img_facemesh, "", (20, 150), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 0, 255), 1)
            else:
                cv2.putText(img_facemesh, "freeze", (20, 150), cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 0, 255), 1)
